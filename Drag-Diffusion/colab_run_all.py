"""
Self-contained Colab runner for Drag Diffusion.
...
The script installs dependencies, downloads the two Hugging Face models,
runs baseline and noise-shift relocation, shows the outputs, and saves files
under ./colab_results.
"""
...
except Exception as exc:  # pragma: no cover - intended for Colab use
    raise RuntimeError("This file is intended to run inside Google Colab.") from exc


MODEL_ID = "sd2-community/stable-diffusion-2-1-base"
INPAINT_MODEL_ID = "sd2-community/stable-diffusion-2-inpainting"

PROMPT = "a photo of the same scene with the selected object moved to the target location"
SEED = 42
NUM_INFERENCE_STEPS = 4
SDEDIT_STRENGTH = 0.35
GUIDANCE_SCALE = 5.0
FEATHER_SIGMA = 2.0
RUN_BASELINE = True
...
def upload_single_file(label: str) -> str:
    print(label)
    uploaded = files.upload()
    if not uploaded:
        raise RuntimeError(f"No file uploaded for: {label}")
    return next(iter(uploaded.keys()))


def masked_psnr(img1: Image.Image, img2: Image.Image, mask: Image.Image = None) -> float:
...
def invert_union_mask(source_mask: Image.Image, target_mask: Image.Image, size) -> Image.Image:
    src = np.array(source_mask.resize(size, Image.Resampling.NEAREST).convert("L")) > 127
    tgt = np.array(target_mask.resize(size, Image.Resampling.NEAREST).convert("L")) > 127
    bg = (~src & ~tgt).astype(np.uint8) * 255
    return Image.fromarray(bg, mode="L")


def show_results(original, source_mask, target_mask, composite, baseline, ours):
    images = [original, source_mask.convert("RGB"), target_mask.convert("RGB"), composite]
    titles = ["Original", "Source Mask", "Target Mask", "Composite"]
    if baseline is not None:
        images.extend([baseline, ours])
        titles.extend(["Baseline", "Ours"])
    else:
        images.append(ours)
        titles.append("Ours")
...
def main():
    ...
    image_path = upload_single_file("Choose the input image file.")
    print("Upload your source mask PNG.")
    src_path = upload_single_file("Choose the source mask PNG.")
    print("Upload your target mask PNG.")
    tgt_path = upload_single_file("Choose the target mask PNG.")

    image = Image.open(image_path).convert("RGB")
    source_mask = Image.open(src_path).convert("L")
    target_mask = Image.open(tgt_path).convert("L")

    os.makedirs("colab_results", exist_ok=True)

    pipe = ObjectRelocationPipeline(device=get_device(), local_files_only=False)
    ...
    if RUN_BASELINE:
        ...
        baseline.save("colab_results/baseline.png")
    else:
        composite = create_composite(
            image.resize((pipe.image_size, pipe.image_size)),
            source_mask.resize((pipe.image_size, pipe.image_size)),
            target_mask.resize((pipe.image_size, pipe.image_size)),
        )

    print("Running noise-shift method...")
    ours, composite = pipe(
        image,
        PROMPT,
        source_mask,
    ...
    resized_original = image.resize((pipe.image_size, pipe.image_size))
    resized_source = source_mask.resize((pipe.image_size, pipe.image_size))
    resized_target = target_mask.resize((pipe.image_size, pipe.image_size))

    resized_original.save("colab_results/original.png")
    resized_source.save("colab_results/source_mask.png")
    resized_target.save("colab_results/target_mask.png")
    composite.save("colab_results/composite.png")
    ours.save("colab_results/ours.png")

    bg_mask = invert_union_mask(resized_source, resized_target, resized_original.size)
    print("Background PSNR vs original:")
    if baseline is not None:
        print(f"  baseline: {masked_psnr(resized_original, baseline, bg_mask):.4f}")
    print(f"  ours    : {masked_psnr(resized_original, ours, bg_mask):.4f}")

    show_results(resized_original, resized_source, resized_target, composite, baseline, ours)
    print("Saved outputs in ./colab_results")
